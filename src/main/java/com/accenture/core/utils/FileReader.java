...
public class FileReader {
    ...
    /**
     * Synchronize reading of the file so this FileReader can be shared between
     * multiple threads.
     */
        ...
        if (!synchronizedReads) {
            if (nextLine != null)
                throw new CoreException(
                    "Cannot check for end of file while there is still data in the input buffer.  Call readLine() first.");
    ...
    public String readLine() throws Exception {
        if (!synchronizedReads) {
            if (nextLine == null)
                throw new CoreException(
                    "No data to read.  Either end of file found or eof() has not been called yet.");
