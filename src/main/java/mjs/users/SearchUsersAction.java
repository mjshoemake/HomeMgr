...
public class SearchUsersAction extends AbstractAction {

    public static final String GLOBAL_FORWARD = "/SearchUsers";
    ...
    public ActionForward processRequest(ActionMapping mapping,
        ...
        metrics.startEvent("SearchUsers", "action");

        try {
            SearchManager dbMgr = new SearchManager();
            clearBreadcrumbs(req);
            addBreadcrumbs(req, "User List", "../SearchUsers.do");
            ...
            String mappingFile = "/mjs/users/UserSearchMapping.xml";
            ...
            if (!listFound || searchParamsChanged > 0 || paginatedListDirty) {
                ...
                try {
                    dbMgr.open();
                    list = dbMgr.searchUsers(params, 30, 450, GLOBAL_FORWARD);
                    list.setFieldDefs(dataMapping);
                    list.setFilterAction("/home/SearchUsers.do");
                    dbMgr.close(true);
            ...
            metrics.endEvent("SearchUsers", "action");
