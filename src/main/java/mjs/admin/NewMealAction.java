...
public class NewMealAction extends AbstractAction
{
   /**
    * Execute this action.
    *
   ...
   {
      metrics.startEvent("NewMeal", "action");
      try
      {
         addBreadcrumbs(req, "New Meal", "../NewMeal.do");
         return (mapping.findForward("success"));
      } catch (java.lang.Exception e) {
         ActionException ex = new ActionException("Error trying to create a new meal.", e);
         throw ex;
      } finally {
          metrics.endEvent("NewMeal", "action");
