...
import mjs.common.aggregation.AbstractArrayList;
 ...
 * pagination functionality so the list can be retrieved from the database once
 * and paginated from there.
 * 
 * @author mshoemake
 */
...
public class PaginatedList extends AbstractArrayList {
    /**
     * The logger used to log out the result set information. This allows the
     * tracing of the result set data to be managed separately from the standard
    ...
    /**
     * The global forward to use for to get back to the current JSP page. This
     * is used by the pagination actions.
     */
     ...
     * @param globalForward The global forward to use for to get back to the
     *            current JSP page. This is used by the pagination actions.
     */
     ...
     */
    public void appendList(Collection list) throws CoreException {
        Object items[] = list.toArray();
        int i = 0;

        log.debug("Adding beans to PaginatedList...");
     ...
     */
    private boolean isRecordOnThisPage(int recordNum,
                                       int startOfPage) {
        return recordNum >= startOfPage && recordNum <= (startOfPage + pageLength - 1);
    }

    /**
     * Go to the specified record number and update the pagination accordingly.
     * 
     * @param newRecord
     */
