...
import mjs.common.model.FormatUtils;
 ...
 * database. It sends commands to the driver which then handles the actual
 * communication with JDBC and the database.
 * <p>
 * 
 * The AbstractDataManager contains the data manager helper methods. These can
 * be used directly for more control over the process. Or, one of the
 * DataManager child classes can be used instead to take advantage of the
 * simpler interface. To use directly, see a child class for examples.
 */
...
public abstract class AbstractDataManager {
    ...
    /**
     * The log4j logger to use when writing log messages to the "Core" category.
     */
    ...
    /**
     * The database connection that the transaction's queries will be executed
     * against.
     */
        ...
        try {
            ...
            if (connection == null)
                throw new DataLayerException(
                    "Unable to open transaction.  Connection received from connection pool is null.");
            ...
            throw new DataLayerException(
                "Unable to open transaction.  Failed to set Connection AutoCommit.", se);
        } catch (Exception e) {
     ...
     * values of the bean. This method accepts the location to the mapping file
     * to load.
     * 
     * @param bean Object
     ...
     * @param value Object
     * @param fieldDef The new PreparedStatementParam value.
     * @param index int
     * @throws DataLayerException
     */
    ...
    /**
     * Generates the SQL to use to store a bean of this class in the database.
     * This query should be used with prepared statements only.
     * 
     * @param table String
     * @param mappingFile String
     * @param type Class
    ...
    public String getInsertSqlForPreparedStatement(String table,
        ...
        try {
            if (driver == null)
                throw new DataLayerException(
                    "Error populating PreparedStatement attributes.  Driver is null.");
     ...
     * @param keyAlreadyPopulated Description of Parameter
     * @return The value of the InsertSqlForPreparedStatement property.
    ...
    /**
     * Generate the SQL to use to update a bean of this class in the database.
     * This query should be used with prepared statements only.
     * 
     * @param table Description of Parameter
     * @param mapping Description of Parameter
     ...
     * @return The value of the UpdateSqlForPreparedStatement property.
     * @exception DataLayerException Description of Exception
     */
    protected String getUpdateSqlForPreparedStatement(String table,
                                                      OrderedMap mapping,
                                                      Class type,
    ...
    /**
     * Generate the SQL statement used to load beans of the specified type from
     * the database.
     * 
     * @param table Description of Parameter
     * @param type Description of Parameter
    ...
    /**
     * Generate the SQL statement used to count rows in the specified table
     ...
     * 
     * are present before the conversion are escaped with another single quote.
     * 
     * @param obj the Object to wrap.
     * @return the quoted Object or null if the initial Object was null.
     */
    ...
    /**
     * Wraps an Object with single quotes and % signs to use in partial query
     * requests.Single quotes that are present before the conversion are escaped
     * with another single quote.
     * 
     * @param obj Object to be wrapped
     * @return the wrapped Object or null if the initial Object was null.
     */
    ...
    /**
     * Wrap the provided Object with single quotes. A percent siqn is also
     * appended to allow partial search queries. Single quotes that are present
     * before the conversion are removed.
     * 
     * @param obj the Object to wrap.
     * @return the quoted Object or null if the initial Object was null.
     */
    ...
    /**
     * Searches an object for single quotes and escapes them with another single
     * quote.
     * 
    ...
    /**
     * Loads the mapping file at the specified location and stores the data in a
     * Hashtable where the key is the field name and the value .
     * 
     * @param mappingFile String
     * @return Hashtable
     * @exception DataLayerException Description of Exception
    ...
    public OrderedMap loadMapping(String mappingFile) throws DataLayerException {
        try {
            if (mappingFile == null)
                throw new DataLayerException(
                    "Error loading mapping file.  Mapping file path is null.");
    ...
    /**
     * Convert an attribute to it's string representation to be used with an
     ...
     */
    /*
     * protected String getInsertSql(String table, Hashtable mapping, Object
     * bean, boolean keyAlreadyPopulated) throws DataLayerException { Method
     * method = null; Object[] args = {}; Object newobj = null; StringBuffer sql
     ...
     * 
     * if (pds == null || pds.length == 0) throw new
     * DataLayerException("Error creating Insert SQL. Bean Discriptors are missing."
     * );
     * 
     * boolean first = true;
     * 
     * // Append field list. for (int i = 0; i < pds.length; i++) { if
     * (mapping.containsKey(pds[i].getName().toLowerCase())) { String fieldName
     * = pds[i].getName().toLowerCase(); Field fieldDef =
     * (Field)(mapping.get(fieldName));
     * 
     * 
     * if (fieldDef.getType().equalsIgnoreCase("key") && ! keyAlreadyPopulated)
     * { // Key is not already populated. Get the next value in // the insert
     * statement. if (! first) sql.append(", "); else first = false;
     * sql.append(pds[i].getName()); } else { // Do NOT put in field name if
     * field value is NULL... method = pds[i].getReadMethod(); newobj =
     * method.invoke(bean, args); if (newobj != null) { if (! first)
     * sql.append(", "); else first = false; sql.append(pds[i].getName()); } } }
     * } sql.append(") values (");
     * 
     * first = true; // Append values. for (int i = 0; i < pds.length; i++) { if
     * (mapping.containsKey(pds[i].getName().toLowerCase())) { String fieldName
     * = pds[i].getName().toLowerCase(); Field fieldDef =
     * (Field)(mapping.get(fieldName));
     * 
     ...
     * 
     * sql.append(sequence); sql.append(".NEXTVAL"); } else { // Process the
     * non-key fields. method = pds[i].getReadMethod(); newobj =
     * method.invoke(bean, args); if (newobj != null) { if (! first)
     * sql.append(", "); else first = false;
     * sql.append(attributeToString(pds[i], newobj, fieldDef)); } } } }
     * 
     * sql.append(") ");
     * 
     * } catch (Exception e) { throw new
     * DataLayerException("Failed in getInsertSql().", e); }
     * 
     * return sql.toString(); }
     */
